...
![浮动列表](docs/screenshot-float.png)

浮动列表常驻桌面右下角，每个账号两条状态条（5h / 周），50–80% 通体黄、≥80% 通体红。
底部 ⟳ 立即刷新、⚙ 打开设置；点账号行也可打开设置编辑。
...
| 浮动列表 · 账号行 | 每账号两条状态条（5h / 周）+ 已用百分比 + 重置倒计时；点击打开设置编辑 |
| 浮动列表 · 底部 | ⟳ 立即刷新 · ⚙ 设置 |
| 托盘左键 | 显示 / 隐藏浮动列表 |
| 托盘右键 | 菜单：显示/隐藏浮动、打开设置、退出 plan-watch |
| 设置窗 · 底部 | 退出 plan-watch 按钮 |
